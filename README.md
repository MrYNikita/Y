...
- Улучшения
  - ject
    - Реализован заявленный ранее equal.
    - Документация обновлена для каждой функции.
  - YBasic
    - Изменена документация.
    - Большинство функций ject доступны для экземпляров класса.
- Добавления
  - ject
    - F - clone
    - F - equal
...
- Перемещения
  - ject
    ...
    - getDeep -> getProperty
- Исправления
  - Корректировка документации, названий, версий.
***
## Комментарий

### Ject
Нововведения модуля Ject делают возможными заявленные ранее возможности и позволяют всем прочим объектам, наследуемым от YBasic получить весь функционал ject модуля.

### Клонирование
До последних патчей называлось копированием. Клонирование или clone функция позволяет скопировать объект js вплоть до структуры его внутренних ссылок (сохранение внутренней ссылочной структуры).

### Сравнение
Сравнение было заявлено, но не было реализовано. Теперь функция equal позволяет сравнивать свойства объектов между собой, делая вывод об их схожести.

### Пути неисповедимы
Новое обновление модуля OS для путей и файлов включает в себя не только обновление документации, но и добавление новых возможностей. Помимо прочего теперь модуль работает в едином стиле: все пути формируются одинаково и работают также. Немного о новинках далее:

### YPath
Класс путей позволяет удобно работать с путями. Экземпляры, обернутые над строкой пути, позволяют проверить путь на существование, нормализовать его, внести изменения, добавив дополнив его.

С помощью YPath можно использовать весь функционал исходного модуля path без необходимости испортировать его функции.
### path
Пути теперь приведены к единой форме, что означает, что теперь каждый путь абсолютный и что он заканчивается без /. Модуль позволяет нормализовывать пути, убирая из них дубли символов, типа /. Также пути теперь проходят регулярную нормализацию. практически во всех функциях модуля.

Модуль также научился складывать пути между собой. Используется для этого все та же самая нормализация. Нормализованные пути - гарантия успешной проверки и корректности.

### YFile
Были обновлены. Обновилась не только документация, но и внутренние особенности класса. Например, теперь класс не сохраняет отдельно название файла, его локацию и расширение - они извлекаются методами. Хранить их не имеет смысла - вместо этого теперь YFile хранит в себе YPath, как полный путь до нужного файла. Запись в файл и его проверки планируется сделать таким образом, чтобы они могли в том числе пробрасывать исключения.

### YTerminal
Терминалы обзавелись ворохом новых возможностей, такими как загрузка состояния и его сохранение, возможностью добавлять передаваемые элементы и обрабатывать разнообразные события (пока что переходы). Обо всем подробнее в правильной последовательности ниже:

### Передаваемые элементы
Работа над демонстративной версией YTerminal показала, что зачастую многие элементы дизайнерского характера, такие как YHeader не требуется перерисовывать множество раз - многие следующие интерфейсы пожелают оставить их, вместо повторного занесения или клонирования. Так было решено разобраться с данной проблемой.

Решение было найдено в передаче. По сути своей, элементы, которые помечаются, как transfer, передаются в следующий интерфейс, но не занимают место в его элементах, не дублируются, а элоементарно отрисовываются, все также сохраняясь в их начальном интерфейсе. Плюсы такого подхода:
- Нет необходимости дублировать элементы или отрисовывать вручную на разметке
- Отсутствие дублирования устраняет проблему избыточного потребления памяти

### Обработка событий
Некоторые новые элементы (в том числе и один демонстративно добавленный) могут требовать изменений в зависимости от определенных событий терминала. Это означает, что на такие события должны вызывать обработчики, способные сделать требуемые элементами действия.

Для этого были добавлены поля handlers для терминала. Каждый такой handler привязан к одному конкретному событию. На данный момент таких событий в терминале два: переход и возврат - следовательно и обработчиков сейчас два, но это концепция их добавления ясна, если обратить внимание на код.

### Сохранения состояний и загрузка для терминалов
С помощью внутренних protected методов save и load терминал может не только запоминать своё текущее состояние, но и загружать его в следующих сессиях, для того, чтобы не начинать работу занаво для программ сессионного характера.

На текущий момент времени загрузка и сохранение помогают запомнить местонахождение в интерфейсах терминала и перейти к ним после перезагрузки терминала.

### Элемент терминала YLocation
Демонстративный пример того, как реализован элемент указания локации. Данный элемент запоминает переходы по интерфейсам, отрисовывая путь от начального интерфейса вплоть до текущего.

### stringUnify и stringUnifyBySymbol
Новые функции модуля string, призванные не только добавить новый функционал, но и помочь модулю YPath в его нормализации.

Функция stringUnify позволяет найти последовательные дубли символов в строке и свести их к одному повторению.

Отличие unifyBySymbol от unify заключается в указании символов, для которых необходимо искать дубли.
