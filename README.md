...
### config как часть структуры модуля
Модули представленные ранее как совокупность двух файлов module.mjs и class.mjs дополняются третьим функциональным файлом - config.mjs. Это означает, что каждый отдельный модуль будет впредь содержать свой отдельный config.mjs файл со всеми настройками, необходимыми для его работы.

Такие файлы представлены единым экспортом по умолчанию, предоставляющим объект настроек.

### default заполнители
Экспортируемые конфигурациями объекты могут содержать специальные default свойства. Эти дефолты позволяют определять значения по умолчанию для классовых экземпляров модуля. Благодаря тому, что данные дефолты хранятся в объекте-конфигураторе, по ходу работы программы их можно будет изменять динамически кодом, что позволит новым экземплярам сразу перенять необходимые свойства.

Пример работы следующий. Существует класс YPerson с полем Name:
```js
class YPerson extends FPerson {

    name;

}
```
Также существует объект-конфигуратор класса Person:
```js
export default {
    defaultName: 'Guest',
}
```
Цепочка BDVHC классовой цепочки SDIMFY самостоятельно назначит данному полю значение по умолчанию как `Guest`. Какую возможность это открывает? Теперь экспортируемый ранее объект-конфигуратор носит название config. К нему можно обращаться и изменять его поля, которые будут совпадать с полями в файле конфигуратора. Так можно изменить поле defaultName на `Tom` и теперь каждый следующий экземпляр класса YPerson будет по дефолту содержать поле с именем `Tom`.

### Обновление звена классового звена B цепочки BDVHC
Теперь B содержит блок finally. Также звено, как это и требовалось ранее, запускает все последующие звенья.

### Terminal: Bind, Comb
На данный момент терминалы обзавелись новыми классами: Bind и Comb - оба класса позволяют определять привязки и комбинации как объекты, с четко заданными полями не привязанными к индексам.

### Bind
Класс позволяет задавать привязки, обозначая их Comb (сочетанием). Также привязки теперь могут содержать не одну а множество исполняемых функций. В случаях, когда к уже существующей привязке необходимо добавить новую функцию, достаточно лишь также вызвать метод дополнения привязки: метод сам добавит к привязке её функцию.

### Comb
Comb класс описывает комбинацию, на которую будет реагировать привязка и по которой её можно будет опознать. Функционально, они также теперь позволяют легко и просто описывать ситуации, когда какая-либо нажатая клавиша используется с shift или/и с ctrl. Удобство и простота заключаются в том, что такие сочетания считаются отдельной комбинацией - теперь нет потребности в существующей привязке самостоятельно описывать кодом случаи зажатия shift или/и ctrl.

### Receiver
Класс Receiver теперь используется для единого обозначения принципа работы приёмников, которыми раньше выступали терминалы, интерфейсы и интеракторы.
