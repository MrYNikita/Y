...
### Шаблонные классы в привязках YTerminal
Новая возможность, изученная в JSDoc, позволяет гибко указывать, с какими сущностями будут работать функции привязок.

### Новый стиль элементов YTerminal
Данные стили предназначены для улучшения визуализации и удобства использования. В основном были придуманы новые рамки, в которых может быть указан заголовок, подобно тегу legendary. Данный стиль планируется реализовать в модуле YElement, как набор функций, позволяющих обрамлять элементы.

### Индикатор элемент YIndicatorKey
Данный элемент позволит наблюдать, какая комбинация была сформирована прослушивателем, что может помочь в отладке или может пригодится как элемент какого-либо интерфейса.

### Новые возможности YInput
Теперь данный элемент не принимает привязки со значением дефолт. Вместо них добавлены новые виды привязок: `all`, `text`, `number` и `operate` - из всех реализован только `text`.
Указав данные привязки, можно обрабатывать значения по умолчанию, если таковые имеют место быть. Например, вы желаете обработать нажатие клавиши, для дополнения текста - используйте привязку с кодом `text`, чтобы не обрабатывать каждую клавишу по отдельности.

`text` не пропустит нажатия таких клавиш, как `up`, `f1` и им подобные.

### YResponse
Все Receiver'ы ожидали ранее ответ от сигнала, как boolean значение. Такой ответ позволял понять, принял ли хоть один приёмник отправленный ему сигнал, но в то же самое время, этот ответ был недостаточно функционален. Он не позволял понять каждому предыдущему приёмнику, стоит ли ему каким-то образом реагировать на принятие сигнала. Если ранее сигнал отправлялся, находился и не требовал перерисовки, то она осуществлялась независимо от того, было ли это необходимо. (В первой версии не было такой проблемы, но только потому что код для каждого приёмника был уникальным, а не наследуемым. Однако, даже такой вариант требовал весомых доработок)

Решить данную проблему были призваны YRespons'ы. Данный класс позволяет формировать более сложный ответ и возвращать его всем приёмникам, чтобы дать им понимание о последующих для них действиях.
