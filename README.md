...
- Улучшения
    - path
        - Добавлено звено getIn для поиска среди указанного в качестве аргумента перечня путей.
    - directory
        - getFile
            - Добавлена фича с поиском файла или директории через указание строки. Если в качестве аргумента нет возможности передать каталог файлов указанной директории, то можно передать путь самой директории. Это приведет к считыванию всех путей директории в звене обработчика и будет получен массив всех путей директории.
- Добавления
- Исправления
    - YDirectory
        - getFile, getDir
            - Оптимизирован поиск файлов. Теперь поиск осуществляется за счет перебора путей директории, которые были считаны ранее. Это убирает избыточную необходимость проводить повторное считывание всех путей проекта и ускоряет поиск за счет сокращения кол-ва перебираемых вариантов.
