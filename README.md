...
- Улучшения
    - os
        - get
            - Теперь функция запоминает корневую папку проекта.
    - ject
        - fill
            - Теперь fill может назначать для полей объектов любые значения, отличные от undefined.
- Добавления
    - ject
        - Добавлена ФЦ copyDeep для глубокого клонирования объектов.
- Исправления
    - os
        - ФЦ get работает от корневой директории проекта.
    - YFile
        - Исправлена ситуация передачи строчного аргумента при которой не было возможно найти нужный файл.
        - Убрана избыточность в звене b.
    - ject
        - Переименована функция deepGet на getDeep.
        - Переименована функция deepSet на setDeep.
        - Переименована функция deepChange на changeDeep.
        - Переименована функция deepSupplement на supplementDeep.
    - string
        - castToNumberPhone
            - Испралена ошибка потери последнего символа в результате преобразования.
    - YString
        - Исправлен метод paste. Теперь вставка с учетом префикса и постфикса работает корректно.
        - Переименован метод changePasteEnd в changePostfix.
        - Переименован метод changePasteStart переименован в changePrefix.
***
## Комментарий
***
## Планы
