...
- Улучшения
  - ject
    - YReptBlock
      - Теперь оптимизировано указание аргументов для блоков.
      - Теперь блоки могут хранить множество текстовых значений, а не единственное указанное.
      - Теперь значения блоков разделяются символом переноса строки для тех случаев, когда значений более одного.
      - Теперь блоки при цепном отображении сталкиваясь с идентичными блоками отчета в цепочке будут извелкать из него уникальные строки и дополнять ими отчет.
  - array
    - getRandomElement
      - Теперь может вернуть несколько случайных неповторяющихся элементов из одного списка.
  - string
    - generateWord
      - Теперь при указании части конца будет добавлять её в значение, заменяя конечный символ.
      - Теперь при указании части начала будет добавлять её в значение, заменяя ею первый символ.
    - castToYReport
      - Теперь функция убирает избыточные пустые строки отчета.
      - Теперь функция отображает null, как _.
      - Теперь функция отображает undefined, как ?.
- Добавления
  - ject
    - Добавлена ФЦ получения цепочки наследований для объекта - getInheritance.
  - string
    - Добавлена ФЦ вставки значений в поля исходной строки - stringInsert.
  - regexp
    - Добавлена ФЦ вставки значений в поля исходного регулярного выражения - regexpInsert.
    - Добавлена ФЦ превращения строки в регулярное выражение с правилами Y - castFromString.
- Исправления
  - regexp
    - YRegExp
      - Исправлена ошибка преобразования строки в регулярное выражение - некоторые символы, используемые регулярными выражениями внутри строки воспринимались как части регулярных выражений. Ошибка была устранена экранированием всех таких символов.
  - string
    - generateWord
      - Исправлена ошибка повторения согласных.
***
## Комментарий
**YExtension**
