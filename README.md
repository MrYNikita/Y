...
### YError
Ошибки стали полноценным модулем. Ранее они не описывались и не использывались, а потому и не развивались. Сейчас их необходимость имеет вес, поэтому они были включены в разработку. Класс YError позволяет создавать ошибки отличные от классических Error. Данные ошибки не наследуются от него. Эти ошибки могут отображать рекомендации, причины, уточнения, естественное описание.

### Новый файл стурктуры error
Данный файл предназначен для коллекционирования ошибок. Преимущества данного подхода заключается в том, что все потеницальные ошибки, которые могут быть пораждены модулем или классом могут быть описаны в единном месте, вместо того, чтобы быть разбросанными по коду. Новые ошибки легко добавляются и к ним легко обращаться. Файл не обязательный.

### Новейший YMany
Новейший класс множеств перенял новые стандарты и стал более функционально полным.

### array: bring и equal
Функции массивов bring и equal позволяют сравнивать массивы и дополнять их до нужного размера.
