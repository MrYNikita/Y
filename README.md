...
- Удаления
    - string
        - Удалено звено replacePointed. Теперь его функционал реализован в звене replace.
        - Удалена именованная скобочная группа `ra`, её функционал заменен на флаг `g` в регулярном выражении замены.
    - YString
        - Удален метод replacePointed.
- Улучшения
    - YElement
        - constructor
            - Добавлена возможность передавать в качестве аргумента строку создания элементов.
    - string
        - replace
            - Улучшен алгоритм поиска совпадений.
            - Скобочная группа `ra` теперь заменена на комбинацию именованной группы `r` с флагом `g`.
            - Реализовано избегание повторных изменений в уже созданных изменениях.
- Добавления
    - Архитектура
        - SDFY-2.0 - Новая концепция 
    - element
        - Добавлена ФЦ stringDecompose для разьора строки создания элемента.
    - YElement
        - Добавлен класс YElementStyle - таблица стилей.
    - YElementStyle
- Исправления
    - string
        - replace
            - Устранена ошибка многострочного режима замены.
            - Устранена ошибка в алгоритме поиска совпадений для произведенных замен.
...
- [ ] Исправления os.file.directory.
    - [x] Исправить поиск таким образом, чтобы осуществлялся перебор не всех путей проекта, а файлов директории, если она представлена указанным экземпляром.
