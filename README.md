...
- Удаления
    - YServer
        - Удалены логи. Они заменены на журнал.
- Улучшения
    - array
        - remove
            - Добавлена возможность очистки всех элементов массива для случая, когда не указаны элементы удаления.
- Добавления
    - log
        - Добавлен класс YLog.
        - Добавлен класс YNotice.
        - Добавлен класс YSection.
    - YAPI
...
- Исправления
    ...
    - element
        - create
            - Исправлен объект-носитель аргументов, который не учитывал возможность передачи вложенных элементов.
        - stringDecompose
            - Исправлена ФЦ, которая ранее не передавала информацию о вложенных элементах, но распознавала их.
    - server
        - Исправлена ошибка некорректной отправки исполняемых js модулей. Теперь они отправляются как application/javascript.
    - YAPI
        - Исправлена отправка файлов. Теперь она осуществялется с помощью потоков fs.
        
***
## Комментарий
**Логи**    
Для создания одноформатных и информативных логов используется модуль лог, в состав которого были добавлены три новых класса. Общее предназначение модуля теперь сводится к созданию журналов логирования. Данные журналы содержат всю уведомительную информацию, которая может быть изучена конечным пользователем модуля. Журналы состоят из секций. Каждая секция является сборником соотвествующих уведомлений. Зачем нужны разделы? Они позволяют: разделять уведомления по видам, контролировать численность уведомлений, обозначать их особыми метками или символами.

**Журнал уведомлений**  
Журнал - экземпляр класс YLog. Представлен совокупностью секций. Журнал может динамически создавать или удалять секции логов, получать последовательность логов в соотвествии с датой их создания. При необходимости журнал может быть записан в раздел журналирования проекта. Запись осуществляется за счет потоков. При необходимости она может быть начата или прервана вызовами соответсвующих методов.

Журналы планируется использовать для отслеживания состояний отдельных экземпляров, таких как YServer. Он может быть использован для получения информации об ФЦ. Журнал также может создаваться для привязки к любым другим объектам на усмотрение пользователя.

**Секции**
Секция - это раздел уведомлений. Он объединяет их, позволяя отличать по признаку принадлежности. Каждая секция имеет свою метку, приоритет и символ. Приоритет определяет важность отображения данных элементов. Метка определяет ключевую строчку, по которой данные элементы можно было бы отличать друг от друга. Символ является эквивалентом метки, который может отображаться взамен ей для визуального опознования.

Формат отображения данных для секционных уведомлений отличается от обычных и выглядит так: t - l\s - d - где t - дата, l\s - метка или символ и d - данные.

**Уведомления** 
Уведомление - это объект, содержащий информацию вида t - d, где t - время, а d - данные. Уведомление может существовать независимо от журналов и секций. Каждое уведомление может быть соотнесено с противоположным уведомлением.

По задумке, уведомления - это базовый класс для введения уведомительных стандартов.

**POST**
Была успешно реализована поддержка пост запросов. Теперь сервер может не только принимать запросы от клиента, но и получать от него данные. Для создания post запроса достаточно также обратиться к методу post у экземпляра yapi сервера.
