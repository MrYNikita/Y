    ...
    - Модули
        - file
            - directory
                - YDirectory
                    - getFile
                        - Вместо поиска файла от размещения папки теперь поиск начинается от полного пути папки.

***
## Комментарий
Из-за ошибочного кода объекты класса YDirectory или просто директории не могли найти искомые файлы. Старый код подразумевал поиск от локали директории, хотя логично, что поиск файла директории осуществляется не от её местоположения, а от её пути, что и было исправлено. В следтсвии внесения исправлений, в самой незначительной степени были задеты регулярные выражения метода directoryGetFile.

Также был намечен шаг по оптимизации кода поиска файлов в директории. По существу, на текущий момент поиск сводится к перебору всех путей проекта, вместо обычного перебора уже сохраненных в директории путей, что требует правки.
