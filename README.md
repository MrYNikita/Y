...
- Улучшения
  - SDIMFY
    - Теперь при наследовании классов, параметры переданные не через объект-носитель аргументов доходят до последующих классов.
    - Теперь при определении объектных типов для конструктора используется приём объединения объектных типов с исключением общих свойств.
  - string
    - YString
      - Теперь строки при любой вставке производят операцию mesuare.
      - Теперь при имзенении размеров строки происходит изменение её обернутого значения.
      - Теперь установить положение для курсора можно в две координаты - номер символа и индекс строки (пока что работает только при указании размера для строк).
  - ject
    - cli
      - Теперь cli переименован - YTerminal.
      - Теперь терминал отображается как структура скринов.
      - Теперь прослушиванием для терминалов занимается отдельный экземпляр класса прослушивателей.
- Добавления
  - string
    - Добавлена ФЦ для пошаговой вставки подстрок в исходную строку каждые n символов - mesuare.
  - ject
    - YTerminal
      - Добавлен класс YHeader.
      - Добавлен класс YElement.
      - Добавлен класс YIntercat.
      - Добавлен класс YInterface.
- Исправления
  - SDIMFY
    - Исправлена ошибка отображения приватных свойств при создании объекта-носителя аргумента для конструктора.
  - array
    - level
      - Исправлена работа ФЦ, возвращаемое значение более корректное.
***
## Комментарий
**YExtension**
