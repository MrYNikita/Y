...
- Улучшения
    - log
        - YNotice
            - Теперь уведомления обладают свойством, указывающим на их секцию.
            - Если для уведомления указана секция, то уведомление будет отображаться по её правилам.
        - YSection
            - Добавлен метод getNotice, который позволяет получить уведомление согласно правилам секций.
- Добавления
    - string
        - Добавлена ФЦ castToSample для преобразования исходной строки в строку, отображающие специальные символы.
        - Добавлена ФЦ castToYPropertyValue для преобразования исходной строки в оформленную строку стиля `YString`.
- Исправления
    - log
        - YLog
            - Исправлен метод get, который возвращал записи без учета очередности добавления.
            - Исправлен display, работавший на get.
        - YSection
            - Секции указывают уведомлениям, что они им принадлежат.
            - Исправлено добавление новых уведомлений через append - теперь для добавленных через метод уведомлений указывается их секция.
