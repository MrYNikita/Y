...
- Улучшения
  - string
    - replace
      - Теперь корректно отображаются параметры.
      - Теперь изменяет строго первое соответствие.
      - Оптимизирован за счет устранения избыточных операций.
    - replaceAll
      - Теперь есть возможность изменять множество совпадений.
    - YString
      - exec
        - Теперь позволяет выполнять куски кода с участием строки внутри строки.
      - clearPrePostfix
        - Теперь переименован в changePrePostfix.
        - Теперь позволяет разом заменить префикс и постфикс передачей аргументов.
      - pasteTemplate
        - Теперь работает по правилам вставки, то есть через метод paste.
        - Теперь вставляет только существующие шаблоны вставленные через appendTemplate.
      - addTemplate
        - Теперь учитывает вставки.
        - Теперь переименован в appendTemplate.
        - Теперь работает только при указании существенных метки и значения вставки.
    - YTemplate
      - get
        - Оптимизирован за счет изменения алгоритма сбора вставок.
    - YInsert
      - Теперь может принимать значения по умолчанию для вставки в ситуациях, где отсутствует значение вставки.
      - Теперь может возвращать ключ в виде регулярного выражения.
      - Теперь может возвращать значение с учетом того, что это функция.
    - Yserver
      - Теперь переписан до SDIMFY.
- Добавления
  - Добавлен модуль date.
  - Добавлен класс YDate.
  - string
    - replaceAll
      - Добавлена ФЦ для замены всех приведенных фрагментов на указанное значение.
    - YInsert
      - Добавлен метод getKey.
      - Добавлен метод getValue.
      - Добавлено свойство deafult.
    - YString
      - Добавлен метод exec.
- Исправления
  - YRegExp
    - Исправлена ошибка с присвоением фантомных флагов.
  - string
    - find
      - Исправлена критическая ошибка поиска, при которой не учитывалось значение пустой строки.
    - filter
      - Исправлена функция замены.
    - YTemplate
      - get
        - Исправлено указание некорректной функции для замены значений.
    - element
      - Исправлен поиск элементов.
***
## Комментарий
**YExtension**
