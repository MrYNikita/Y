...
- Улучшения
    - YServer
        - Улучшен до WebSocket.
    - html.element
        - Было улучшено регулярное выражение поиска строки создания элементов.
- Добавления
    - YLog
        - Добавлена ФЦ getVis для получения уведомлений в области видимости.
        - Добавлено свойство области видимости журнала.
    - YServer
        - Добавлена отправка WebSocket коротких сообщений для клиентов.
        - Добавлены постоянные соединения.
        - Добавлена регулярная проверка соединений.
    - web
        - Добавлен модуль fetch.
        - Добавлен модуль client.
    - web.client
        - Добавлен класс YClient.
- Исправления
    - web
        - Добавлен файл модуля html.
    - YString
        - Основной курсор сдвигается при вставке шаблона.
***
## Комментарий
**Свой WebSocket**  
Новая версия представляет свой вариант реализации WebSocket сервера без сторонних зависимостей. Стоит отметить, что на данный момент сервер уступает аналогам, но это не означает, что такая ситуация будет продолжаться дальше - Yserver будет развиваться и стремиться к достижению новых результатов, как функциональных, так и производительных.

Что теперь может сервер? Как и любой WebSoсket сервер, YServer может поддерживать постоянное соеинение со своими клиентами, по которому они могут обмениваться сообщениями. Клиент не нуждается в постоянных запросах к серверу - все нужно сервер передаст сам. В свою очередь получение данных от клиента так же может быть сведено к отправке данных по постоянному соединению.

**Client**  
Для взаимодействия с YServer на страницах будет создаваться (или может быть создан) YClient. Данный класс позволяет взаимодействовать с сервером и запросами. Каждый экземпляр будет обладать собственными наборами функций для взаимодействия с сервером и формированием запросов без необходимости описания сложного кода. Формирование заголовков, определение типа отправляемых данных и статусов запросов будет также переведено на YClient.

**YFrame**
Данный класс ещё не заявлен, но уже сейчас над ним ведется разработка. Экземпляры данного класса будут ответсвенны за передачу данных клиенту и серверу соответсвенно. YFrame будет определять маску, заниматься распознованием сообщения, шифрованием.
...
- [x] Аналог WebScoket/Включение WebSocket в проект.
- [ ] WebSocket - реал