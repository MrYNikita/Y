...
Текущая структура вносит единство в общую структуру модулей, позволяет понять иерархию модулей и открывает новые функциональные возможности. Вид структуры на примере List:

- list <название модуля>
  - module.mjs <код модуля>
  - class.mjs <класс модуля>
  - Tag <вложенный модуль>
    - module.mjs
    - class.mjs

Данная структура позволяет добавлять новые файлы с функциональными возможностями, соответсвующими их именам. В будущем планируется добавить для каждого модуля индивидуальный пакет, отдельный конфигуратор на основе общего конфигуратора.

### Изменения сниппетов
Сниппеты теперь указывают точное местонахождение модуля, также изменен метод их указания, из-за чего удалось добиться некоторой оптимизации и минимизации их файла.

### Ject.adopt
Классы наследованные от YBasic по дефолту содержат метод принятия, который с этих пор называется adopt. Метод принятия заменил собой функцию, которая требовала регулярного импорта (jectFill). Adopt выполняет все ту же самую роль, однако теперь его не нужно импортировать отдельно: достаточно лишь наследоваться от класса, имеющего в цепочке наследований YBasic класс.

### List теперь не List, а Many, в то время как List реализован новым классом.
Теперь списки не представляют собой базовый класс для множеств. Впредь они используются для работы с записями. Для того, чтобы использовать старые возможности List следует обратить внимание на новый модуль Many.

List включает в себя классы, для работы с записями. Записи можно тегировать, назначать им приоритет, делать выборки и не только. Все эти возможности позволяют наследоваться от списка и гибко управлять его содержимым.

### Terminal
Терминал находится в глубокой доработке. Не рекомендуется на данный момент использовать свежую начальную версию. Немного о терминалах и планах на них ниже:

- Бинды будут реализованы объектами.
- Терминалы будут добиваться динамеческого отображения контента.
- Будет разработана общая концепция перехвата сигналов и их передачи.
