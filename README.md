...
- Улучшения
  - SDIMFY
    - Теперь без версии в названии.
  - BDVHC
    - Теперь в типе для r в функциональном звене B учитывается классовое звено D при разборе аргументов-значений.
  - YRegExp
    - Теперь переписан до SDIMFY.
    - Теперь могут добавляться вариации.
    - Теперь задаёт постоянный порядок флагов.
    - Теперь позволяет удалять из имеющихся флагов указанные.
    - Теперь позволяет получать итоговое выражение через get.
    - Теперь позволяет динамично изменять регулярное выражение.
    - Теперь позволяет получать состояние регулярного выражения.
    - Теперь позволяет динамично изменять флаги регулярного выражения.
    - Теперь корректирует флаги, избегая повторений и недопустимых значений.
    - Теперь добавляемые вариации могут быть размещены только в единственном экземпляре.
    - Теперь могут принимать в качестве аргумента для конструктора строки, регулярные выражения и свои экземпляры.
  - string
    - find
      - Теперь поиск при find возвращает строго одно значение.
      - Теперь, если поиск будет безуспешным, то ответом вернется пустая строка.
      - Теперь может осуществлять глубокий поиск. Это когда для одной строки последовательно применяется множество регулярных выражений.
    - findToJect
      - Теперь заполняет итоговый объект через дополнение.
      - Теперь не требуется указывать класс для создоваемого экземпляра - всегда возвращается объект.
    - findVariation
      - Теперь переименована в findVariate.
      - Теперь работает не на match с флагом y, а на exec.
    - castToDate
      - Теперь работает без YString, так как это было избыточно.
    - shield
      - Теперь доработана.
      - Теперь экранирует все специальные символы.
    - YString
      - find
        - Оптимизирован за счет удаления избыточной проверки на массив.
- Добавления
  - YRegExp
    - Добавлен метод get.
    - Добавлен метод change.
    - Добавлен метод display.
    - Добавлен метод changeFlags.
    - Добавлен метод appendFlags.
    - Добавлен метод removeFlags.
    - Добавлен статический метод correctFlags.
  - string
    - Добавлена ФЦ findAll.
- Исправления
  - string
    - remove
      - Исправлено некорректное удаление частей строк.
      - Исправлено дублирование удалемых частей строк.
    - castToDate
      - Исправлена ошибка в указании дня через метод getDay вместо getDate.
      - Исправлена ошибка в указании месяца.
    - castToSample
      - Исправлен поиск спецкода \x1b.
  - SDMIFY-BDVHC
    - Исправлен тип в звене B для r значения.
***
## Комментарий
***
## Планы
