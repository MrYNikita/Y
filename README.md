...
- Добавления
    - Документация
        - Добавлен раздел модулей.
- Исправления
    - Модули
        - path
            - supplement
                - Исправлена ситуация указания имени папки лежащей в корневой директории для которой возникала ошибка.
        - file
            - fileREPart
                - Улучшено регулярное выражение. Теперь оно способно определять все части указанного пути в том числе для корневых путей.
            - directory
                - YDirectory
                    - Исправлена ситуация некоректного определения вложенных путей из-за ошибки в коде.
            - YFile
                - Добавлена фича определения файлов одним аргументом - фрагментом поиска. Укажите в конструктор путь до желаемого файла, вместо передачи объекта-носителя аргументов. B звено функциональной цепочки BDVHC само разберет переданный путь на компоненты и преобразует их в объект-носитель аргументов. 

***
## Комментарий
Web модуль позволяет создать сервер, построенным на функциональных возможностях сторонних модулей. Он позволяет быстро воссоздать сервер на нелокальном хосте, для получения доступа к серверу с разных устройств, подключенных к одной сети.

В совокупности с модулем os, модуль web может разграничить файлы сервера, ускорить и упросить обращение к файлам и работу с ними.

***
## Планы
- [ ] Создание вложенного модуля web.html.  
- [ ] Создание вложенного модуля web.html.style.    
- [ ] Создание вложенного модуля web.html.element.  
