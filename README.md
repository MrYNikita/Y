...
- Улучшения
  - YClient
    - Теперь класс переписан на SDIFMY-1.0.
    - Теперь клиент может прослушивать данные пользовательского ввода.
    - Теперь клиент может остлеживать состояние зажатых клавиш клиента.
  - YString
    - find
      - Теперь строка сохраняет найденное значение в value, вместо возврата.
  - YClient
    - Теперь может формировать отчет.
    - Теперь наследуется от YJect.
    - Теперь отслеживает действия пользователей.
- Добавления
  - string
    - Добавлена ФЦ для преобразования строки в объект.
    - Добавлена ФЦ для экранирования символов в строке.
  - YString
    - Добавлен метод для преобразования строки в объект.
- Исправления
  - Класс SDIFMY-1.0 переименован в SDIMFY-1.1.
  - string
    - convertCamelCaseToDelimetr
      - Исправлена ошибка из-за которой преобразовывалось только первое совпадение.
***
## Комментарий
***
## Планы
