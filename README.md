...
- Улучшения
  - ject
    - copyDeep
      - Работает.
      - Оптимизировано.
      - Учитывает ссылочную структуру исходного объекта. Может сохранить её или сделать все структуры несвязными.
    - string
      - color
        - Изменены значения по умолчанию.
        - Переработаны и оптимизированы некоторые функции.
      - style
        - YStylist
            - stylize
              - Переработан метод stylize.
          - Добавлен функционал для размещений с переносом.
- Добавления
  - string
    - color
      - F - pasteWrap.
    - style
      - YStylist
        - FM - checkEnd.
        - FM - getLastPointByPosition.
        - YStylistMap
          - P - ends.
          - YStylistMapColor
            - FM - pasteStringByWrap.
- Перемещения
- Исправления
***
## Комментарий
### Краткая сводка
Из-за большого числа изменений и праздников часть документации записать не удалось. Обо всем можно будет узнать подробнее в следующих выпусках.

### Терминалы

С момента последнего обновления терминалы были сильно переработаны, в результате чего они стали более оптимизированными, а их конструирование упрощено.

На данный момент создаются новые элементы для терминала, которые помогут влиять на программу и отслеживать состояния самых разных программируемых процессов с помощью элементов отображения.

Сейчас уже могут использоваться элементы (чья иерархия будет пересмотрена), такие как _Header_, _Text_, _Label_ для отображений текста в пределах терминала. Также доступны интерактивные элементы, такие как _Menu_, _Input_, _Form_, _Select_.

Используйте элемент _Form_, чтобы создавать сложные разделы терминала со множеством настроек.

### Listener

С помощью _Listener_ можно прослушивать данные пользовательского ввода. Эти данные воспринимаются терминалом и передаются им для элементов интеракторов, чтобы те смогли адаптироваться под действия пользователя.

### Stylist

Стилисты - давняя идея, заключающаяся в использовании стилизаторов для цветового и не только оформления текста на основе вставок ASCII кода. Строки, проходящие через стилизаторы наполянются цветом.

Основное преимущество стилизаторов заключается в сохранении обозначенных цветовых позиций и удобногь редактирования исходных строк. _YString_ использующий _Ystylist_ может осуществлять вставки цветов и их удаления, без изменения исходной строки. Вы вольны изменять строку не опасаясь повредить цветовую вставку.
## Планы
