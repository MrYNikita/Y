...
- Удаления
  - string
    - YString
      - Удален метод report.
      - Удален метод getReport.
- Улучшения
  - ject
    - YList
      - Теперь листы опираются на общие значения, а их экземпляры по умолчанию не определены.
    - YCursor
      - Теперь курсоры представлены массивом координат.
    - YTerminal
      - Теперь существуют общие привязки.
      - Теперь для многих привязок можеет быть назначено общее действие.
      - Теперь можно указывать, какие привязки будут влиять на отображение.
      - YInteract
        - Теперь существуют групповые привязки.
        - Теперь интеракторы могут вызывать другие привязки.
        - Теперь интеракторы получают данные от прослушивателя.
        - Теперь интеракторы могут в одной привязки последовательно вызвать серию других привязок.
  - config
    - Теперь все конфигураторы размещаются независимо от основного объекта конфигурации.
  - string
      - repaint
        - Теперь добавлена новая палитра цветов для строк.
        - Теперь новые цвета можно добавить через конфигуратор.
        - Теперь цвета в аргументах можно выбирать через intellisence из перечня.
        - Теперь в качестве аргумента можно указать код цвета числом от 0 до 255.
        - Теперь при изменении цвета и фона и переднего плана применяется единая кодовая вставка с параметрами.
      - getColor
        - Теперь возвращает код искомого цвета.
- Добавления
  - ject
    - YTerminal
      - Добавлены общие привязки - binds.
      - Добавлен метод вызова привязки - bind.
      - Добавлен скрытый метод принятия сигнала - receive.
      - YInteract
        - Добавлен класс YForm - групповой интерактор.
        - Добавлен класс YMenu - интерактор выбора действия.
        - Добавлен класс YInput - интерактор ввода.
        - Добавлен класс YSelect - интерактор списка.
  - array
    - Добавлена функция дополнения всех свободных значений массива заданным значением - supplementFill.
  - string
    - Добавлена функция многострочной обрезки - trim.
    - Ystring
      - Добавлен защищенный метод вычисления текущего положения курсора - calculateIndex.
- Исправления
  - string
    - pasteWrap
      - Исправлена ошибка вставки в существующую, но пустую строку, которую счиатли несуществующей.
    - YString
      - repeat
        - Исправлена ошибка некорректного повторения.
    - regexp
      - get
        - Исправлена ошибка включения в выражение символа отстутствия, который присутствует в любой строке.
***
## Комментарий
**String Paste Wrap**
