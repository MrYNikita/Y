...
- Добавления
    - Добавлен логотип.
    - Архитектура
        - SDFY-2.0 - Новая концепция классов.
    - string
        - find
            - Если в качестве поиска указывается единственное регулярное выражение без флага `g`, то метод поймет, что пользователь ожидает получить в качестве ответа строку и вернет таковую, а в противном случае будет null. Если же регулярных выражений несколько или одно, но содержащее флаг `g`, то будет возвращаться массив всех совпадений.
    - YString
        - extract
            - Если в качестве поиска указывается единственное регулярное выражение без флага `g`, то метод поймет, что пользователь ожидает получить в качестве ответа строку и вернет таковую, а в противном случае будет null. Если же регулярных выражений несколько или одно, но содержащее флаг `g`, то будет возвращаться массив всех совпадений.
    - element
        - Добавлена ФЦ stringDecompose для распознования значений строки создания элемента.
- Исправления
    - element
        - Регулярное выражение определения строки создания элементов вынесено в отдельную общую переменную, вместо многоразового повторения в коде.
