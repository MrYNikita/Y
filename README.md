...
- Удаления
    - string
        - Удалена ФЦ extract без потери функциональности. В качестве аналогов используются find и filter.
- Улучшения
    - string
        - find
            - Обычный поиск учитывает именованную группу `f`. Указав её, можно получить найденное соотвествие из найденного фрагмента.
            - Обычный поиск учитывает флаг `g`. При указании данного флага будут найдены и разобраны все совпадения. Все такие совпадения также будут разобраны с учетом именованной группы.
    - documentation
        - Документация теперь не требует указания раздела улучшений, исправлений, добавлений и удалений.
- Добавления
- Исправления
    - YString
        - extract
            - Метод поиска работает за счет ФЦ find.
    - string
        - find
            - Обработчик объекта-носителя аргументов присуждает флаг `y` только тем регулярным выражениям, которые работают в вариационном режиме.
