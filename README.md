...
### YCMD
YCMD стал третьим классом модуля OS, который был обновлен до новой версии классовых цепей SDIMFY. В новом виде данный класс научился многому:

- Работать с YPath.
- Ожидать завершения запущенных инструкций.
- Автоматически прекращать запущенные ранее процессы после завершения выполнения инструкций.

Кроме того, данный класс теперь обладает общедоступным статическим терминалом. Не стоит переживать по поводу затрат оперативной памяти: процесс запускается только в момент необходимости выполнить инструкиции - после он благополучно удаляется. Это означает, что на страте совей работы процесс также не является запущенным. Через статику можно работать с единным терминалом, а через методы статики обращаться к нему без дополнительных вызов.

Статика позволяет получить все преимущества экземпляров `YCMD` без необходимости их создания.

### YPath
Незначительное улучшение в статике класса теперь позволяет используя статический метод getProject получить новый экземпляр YPath, берущий начала от корня проекта.
