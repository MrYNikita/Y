...
- Добавления
    - os
        - path
            - Добавлена ФЦ getProject для получения абсолютного пути до папки проекта.
        - program
            - Добавлен класс YCMD для взаимодействия с терминалом Windows.
- Исправления
        
***
## Комментарий
**Модуль program**  
Данный модуль предназначен для запуска файлов и работой с cmd в windows. С помощью ФЦ executeCmd можно осуществлять запуск cmd с указанными командами, переданными строки. При необходимости можно осуществлять запуск сторонних программ с помощью ФЦ launch.

Планируется создать отдельный YCMD класс, для работы с терминалом в постоянном режиме, без прерывания после отдельно выполненных команд. Также планируется использовать YCMD для возможности исполнения доп.инструкций.

**YCMD**
Благодаря данному классу открывается возможность работать с cmd посредством кода js. Для этого нет необходимости в знании всех тонкостей модуля `child_process` - достаточно лишь воспользоваться классом YCMD. С помощью методов и свойств можно оказывать влияние на консоль, выполнять в ней команды.

Экземпляры данного класса работают с независимыми друг от друга экземплярами консоли cmd. Вся информация о командах может быть сохранена в журналы. Каждый экземпляр берет начало от местоположения проекта.

***
## Планы
- [ ] Модификация path.
    - [ ] Добавление ФЦ прокладки пути от файла до файла.
- [ ] Модификация YFile.
    - [ ] Статика.
    - [ ] Асинхронность.
- [ ] Модификация YNotice.
