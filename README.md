...
- Удаления
    ...
    - verify
        - Очищен весь модуль.
- Улучшения
    - Часть основных классов переведена на SDFY-2.0.
    - element
        - createByString
            - Для определения элементов строки создания элементов используется ФЦ stringDecompose.
            - Для указания строки теперь используется формат `:`...`:`, вместо `:`...`::`.
    - YString
        - Перевод с SDFY-1.0 на SDFY-2.0.
    - YServer
        - Для указания значения `dir` можно просто указать путь до нужной директории. 
- Добавления
    - ject
        - Добавлена ФЦ supplement для дополнения объекта. Дополение объекта отличается от заполнения тем, что исходный объект получит от объекта дополнения только те свойства, которые определены у самого исходного объекта, но являются у него null или undefined.
- Исправления
    - ject
        - fill
            - Исправлен способ обращения к методу fill. Теперь обратиться к нему необходимо через два аргумента без необходимости использовать apply.
    - YString
        - extract
            - Исправлен фрагмент кода, убиравший пустые массивы из результатов поиска.
    - element
        - Создано регулярное выражение elementREId.
        - Создано регулярное выражение elementREType.
        - Создано регулярное выражение elementREText.
        - Создано регулярное выражение elementREOverId.
        - Создано регулярное выражение elementREClasses.
        - Создано регулярное выражение elementREOverTypes.
        - Создано регулярное выражение elementREProperties.
        - Создано регулярное выражение elementREOverClasses.
        - stringDecompose
            - Исправлена распознование элементов строки.
            - Для указания предела перебора указывается значение `]`.
        
***
## Комментарий
**Стили**   
Стили - это объекты-обертки над стилями из таблицы стилей, к которой они привязаны. Любой такой стиль может быть привязан к определенной существующей таблице стилей. После привязки элемент может быть извлечен из данной таблицы или добавлен в неё, если до этого отсутсвовал в ней. Изменение свойств объекта приводит к изменению свойств в привязанной таблице стилей. Добавление некорректных свойств не отобразится на состоянии стиля, так как они будут проигнорированы.

**Генеральные и Побочные стили**
Создавая множество элементов с одинаковыми параметрами будет получен единый для всех генеральный стиль. Этот стиль объединяет данные элементы по их общим признакам до тех пор, пока такие признаки существуют. Для всех модификаций исходных элементов создаются побочные стили. Они позволяют изменить стили исходных элементов и обособить их в отдельную группу.

Благодаря генеральным стилям, можно держать под контролем большую группу элементов и при необходимости получать к ним доступ по обращению к классу даже не смотря на наличие модификаций.

**SDFY-2.0**    
Отличительной особенностью SDFY-2.0 от SDFY является переработка ФЦ BDVHC, в которой вместо привычной цепочки звеньев будет последовательность цепец BDVH-C. Что это означает? Теперь классы SDFY-2.0 будут обрабатывать свой объект-носитель аргументов до звена `H`, после чего обработанный аргумент будет возвращен и использован в `super`. Также после super будет вызываться конечное звено `C`.
