...
- Удаления
- Улучшения
  - Теперь значения @param заменены @arg.
  - ject
    - YJect
      - Теперь объекты по умолчанию сообщают отчетам, что привязываются к ним.
    - YRept
      - Теперь отчет может быть сцеплен с другими отчетами.
      - Теперь отчет хранит ссылку на привязанный к нему объект в свойстве target.
      - Теперь первым аргументом для отчета может быть передан объект привязки.
      - Теперь правила отображения отчета для класса можно указать в сатическом классе, в статическом отчете, чьи блоки будут общедоступные и будут созданы ровно раз.
      - Теперь повышена производительность за счет удаления избыточных повторяющихся операций по созданию блоков отчета для каждого отдельного экземпляра определенного класса.
      - Теперь отчет может заменить значения `yt` на объект привязки. Если после `yt` через точку указывать свойства, то будет вставлено указанное свойство объекта привязки.
    - YList
      - Теперь позволяет удалить курсоры.
      - Теперь позволяет указывать позицию для главного курсора.
      - Теперь позволяет контролировать свойства курсоров через методы.
      - Теперь позволяет сбросить курсоры - сброс отличается от удаления восстановлением нормального состояния, когда главный курсор находится в конце списка, имеет область влияния в единицу и является единственным курсором.
  - array
    - YArrayRank
      - Теперь добавление в ранговых массивах осуществляется конвеерно.
  - string
    - YCLI
      - Теперь терминал управления поддерживает ввод.
      - Теперь в модулях доступен терминал управления.
      - Теперь терминал может выполнять инструкции, описанные для него в свойстве instructions.
      - Теперь терминал способен запоминать ранее введенные команды.
      - Теперь команды, которые были сохранены терминалом, можно выбирать с помощью клавиш стрелок (верх и низ).
    - YString
      - Теперь наследуется от YList.
      - Теперь строки могут быть лимитированы.
      - Теперь свойсвто values наследовано от YList.
      - Теперь свойство value переименовано в values.
      - Теперь может быть назначен поток ввода и поток вывода.
      - Теперь общедоступные шаблоны хранятся в статическом классе.
      - Теперь общедоступные шаблоны хранятся в единственном экземпляре.
      - Теперь при превышении лимита строки будет вставляться символ переноса строки.
      - Теперь символ переноса строки может быть дополнен символами пользователя. Символ переноса сохраняется всегда.
- Добавления
  - Добавлен модуль cli.
  - cli
    - Добавлено свойство текущего состояния команды - command.
    - Добавлено свойства набора инструкций для исполнения - instructions.
  - ject
    - Добавлена ФЦ для получения свойства по переходу через указанные свойства - getByPath.
    - YRept
      - Добавлено свойство отчета для привязки к объекту отчета - target.
    - YJect
      - Добавлено статическое свойство отчета - rept.
  - array
    - Добавлен класс ограниченных (ранговых массивов) - YArrayRank
    - Добавлена ФЦ для создания нового измерения в массиве - arrayLevel.
    - Добавлена ФЦ для создания неравной размерности в массиве - arrayBulk.
    - Добавлена ФЦ для конвеирного добавления элементов в массив - arrayAppendMiss.
    - Добавлена ФЦ для разделения массива на под-массивы по указанному кол-ву элементов - arrayDevideByLimit.
  - string
    - YString
      - Добавлено свойство длины строки - rowLength.
      - Добавлено свойство для потока ввода - input.
      - Добавлено свойство для потока вывода - output.
      - Добавлено свойство завершения строки - rowEnd.
      - Добавлен метод указания текущего значения строки - set.
- Исправления
  - Исправлен порядок подключения скриптов. Теперь в первую очередь поключаются обязательные сценарии.
  - YLog
    - Исправлена ошибка отображения записи при которой происходила полная замена символа, искажающая строку записи.
  - string
    - YString
      - Исправлена архитектурная ошибка общедоступных шаблонов - теперь общедоступные шаблоны вынесены в отдельный массив.
***
## Комментарий
**YExtension**
Начата разработка дополнительной функциональной возможности, позволяющей использовать браузерное расширение YExtension для взаимодействия YModules со сторонними сайтами. На данный момент планируется реализовать перечень базовых возможностей, среди которых будет автообновление страницы по временному таймингу, поиск элементов соотвествующих условию.
